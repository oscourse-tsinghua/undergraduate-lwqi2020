...
function getDistanceBtwP(LatA, LonA, LatB, LonB)//经纬度算距离
{  
	var radLng1 = LatA * Math.PI / 180.0;  
	var radLng2 = LatB * Math.PI / 180.0;  
	var a = radLng1 - radLng2;  
	var b = (LonA - LonB) * Math.PI/ 180.0;  
	var s = 2 * Math.asin(Math.sqrt(Math.pow(Math.sin(a / 2), 2)+ Math.cos(radLng1) * Math.cos(radLng2) * Math.pow(Math.sin(b / 2), 2))) * 6378.137; //路碌禄脴碌楼脦禄脦陋鹿芦脌茂  
	return s * 1000;  
}

function getDistanceByGeohash(geohash1, geohash2) {//geohash算距离
	var vector = getVector(geohash1, geohash2);
	var ans = Math.sqrt(vector[0] * vector[0] + vector[1] * vector[1]);
	return ans;
}

function getVector(geohash1, geohash2) { //geohash算向量
	var ans = new Array();
...
function getProjection(geol1, geol2, geop) {//获得投影点 A为点 BC为线上点
	if (geol1 == geol2) {
		return geol1;
	}
...
function CalculateShift(geohash, latoffset, lonoffset) {
	...
	lat = (lat + latoffset + (1 << (8 * 5 / 2))) & ((1 << (8 * 5 / 2)) - 1);
	lon = (lon + lonoffset + (1 << (8 * 5 / 2))) & ((1 << (8 * 5 / 2)) - 1);
...
function getDisPointToLine(geol1, geol2, geop) {//geohash算点到线距离
	getDistanceByGeohash(getProjection(geol1, geol2, geop), geop);
}

function getVecPointToLine(geol1, geol2, geop) {//geohash算点到线向量
	getVector(getProjection(geol1, geol2, geop), geop);
}

function getDisPointToSegment(geol1, geol2, geop) {//geohash算点到线段距离，考虑投影超出问题
...
function getBase32(geohash) {//geohash转二进制
...
function getLonDelta(geohash) {//当前东西误差
...
function getLatDelta(geohash) {//当前南北误差
	lon = getLonBase32(geohash);
	return deltaLat;
}

function getLatBase32(geohash) {//geohash纬度
	var even = true;
	var latNow = [-90, 90];
	var lonNow = [-180, 180];
...
function getLonBase32(geohash) { //geohash经度
	var even = true;
	var latNow = [-90, 90];
	var lonNow = [-180, 180];
...
/*
function getDistanceByGeohash(geohash1, geohash2) {//碌茫碌陆碌茫戮脿脌毛
